...
1. 支持更多书籍格式：**EPUB、Markdown、MOBI、AZW3、FB2、HTML**
...
pub trait BookEngine: Send + Sync {
    ...
    /// 渲染指定页面（对于 PDF 返回图片，对于 EPUB/HTML 返回内容数据）
...
- [x] 测试 Markdown 阅读（包含代码高亮、表格、图表等）- 需手动测试
...
- [ ] 实现 `src/services/formats/html/HtmlRenderer.ts`
- [ ] 更新 `src/services/formats/index.ts` 映射 `html`
- [ ] 更新 `src/constants/fileTypes.ts` 支持 `.html`
- [ ] 测试 HTML 阅读（处理内联样式与资源引用）
...
执行顺序：**接口定义 → PDF 重构 → Markdown → EPUB → MOBI/AZW3/FB2 → HTML**
