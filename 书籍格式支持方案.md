...
> 版本：1.0
> 日期：2024-12-08
> 作者：Claude & Gemini-3-Pro-Preview 协作方案
...
GoRead 目前仅支持 PDF 格式，采用 Tauri v2 (Rust 后端 + React 前端) 架构：

- **后端**: 使用 `pdfium-render` 进行 PDF 渲染，具有完善的缓存、预加载、并行渲染等优化
- **前端**: `Reader.tsx` 组件负责阅读界面，但直接耦合了 PDF 相关的 Tauri 命令调用

### 1.2 目标

1. 支持更多书籍格式：**EPUB、Markdown、MOBI、AZW3、FB2、TXT**
2. 设计统一的接口抽象，方便后续扩展新格式
3. 复用现有的阅读逻辑（翻页、书签、目录、进度等）
4. 保持现有 PDF 阅读体验不变
...
| 格式 | 后端 (Rust) | 前端 (React) | 渲染策略 | 优先级 |
|------|-------------|--------------|----------|--------|
| **PDF** | `pdfium-render` (现有) | Canvas 渲染 | 图片 | ✅ 已完成 |
| **EPUB** | `epub-rs` (元数据提取) | `epub.js` | HTML/CSS | 🔴 P0 |
| **MD** | 文件读取 | `md-editor-rt` | HTML 预览 | 🔴 P0 |
| **MOBI** | `mobi-rs` (元数据) | `foliate-js` | HTML | 🟡 P1 |
| **AZW3** | 二进制流传递 | `foliate-js` | HTML | 🟡 P1 |
| **FB2** | `quick-xml` (解析) | `foliate-js` | HTML | 🟢 P2 |
| **TXT** | `chardetng` + `encoding_rs` | 纯文本渲染 | DOM 分页 | 🟢 P2 |

### 2.2 Gemini 方案评审与改进

#### ✅ 保留的建议

1. **EPUB**: 使用 `epub.js` 是正确选择，但建议直接使用 `epub.js` 而非 `react-reader`，因为后者更新较慢且定制性差
2. **TXT 编码检测**: `chardetng` + `encoding_rs` 组合是最佳实践
3. **Rust 负责解析，前端负责渲染**: 这是正确的架构思路

#### ⚠️ 需要调整的建议

1. **foliate-js 集成**:
   - 问题：`foliate-js` 不是 npm 包，需要手动管理
   - 改进：将 `foliate-js` 源码放入 `public/lib/foliate-js/` 目录，通过动态 import 使用
   - 或者考虑使用 `@nickolaj/foliate-js` npm 分支（如果存在）

2. **MOBI/AZW3 处理**:
   - 问题：Rust 生态对这些格式支持有限
   - 改进：优先在前端使用 `foliate-js` 处理，Rust 端仅负责文件读取和元数据缓存

3. **缺失的接口抽象**:
   - Gemini 方案未提及如何设计统一接口
   - 本方案将定义完整的接口体系

4. **新增 Markdown 支持**:
   - 使用 `md-editor-rt`（`md-editor-v3` 的 React 版本）进行 Markdown 预览
   - 支持 GitHub 风格 Markdown、代码高亮、Mermaid 图表、KaTeX 数学公式
   - 纯前端渲染，后端仅负责文件读取和编码检测
...
pub trait BookEngine: Send + Sync {
    ...
    /// 渲染指定页面（对于 PDF 返回图片，对于 EPUB/TXT 返回内容数据）
...
│   │   │   ├── txt/                    # TXT 渲染器
│   │   │   │   └── TxtRenderer.ts
│   │   │   └── foliate/                # MOBI/AZW3/FB2 渲染器
│   │   │       └── FoliateRenderer.ts
│   │   └── index.ts                    # 现有服务
│   ├── components/
│   │   ├── Reader.tsx                  # 重构：使用渲染器接口
│   │   └── reader/                     # 现有子组件
│   └── utils/
│       └── ...
├── public/
│   └── lib/
│       └── foliate-js/                 # 新增：foliate-js 源码
├── src-tauri/
│   └── src/
│       ├── formats/                    # 新增：格式引擎层
│       │   ├── mod.rs                  # trait 定义 & 导出
...
│       │   ├── epub/                   # EPUB 引擎
│       │   │   └── mod.rs
│       │   ├── markdown/               # Markdown 引擎（文件读取）
│       │   │   └── mod.rs
│       │   ├── txt/                    # TXT 引擎
│       │   │   └── mod.rs
│       │   └── common/                 # 共用工具
│       │       └── mod.rs
│       ├── format_commands.rs          # 统一命令入口
│       ├── pdf_commands.rs             # 保留兼容（可逐步迁移）
│       └── lib.rs
└── ...
```
...
当前 `Reader.tsx` 中可复用的逻辑：

| 功能 | 可复用性 | 说明 |
|------|----------|------|
| UI 状态管理 | ✅ 完全复用 | `uiVisible`, `isSeeking`, `seekPage` 等 |
| 阅读模式切换 | ✅ 完全复用 | 横向/纵向模式 |
| 书签功能 | ✅ 完全复用 | 通过 `bookmarkService` |
| 目录显示 | ⚠️ 部分复用 | 需要适配不同格式的目录结构 |
| 翻页逻辑 | ✅ 完全复用 | 手势、音量键等 |
| 进度保存 | ✅ 完全复用 | 通过 `bookService.updateBookProgress` |
| 自动滚动 | ✅ 完全复用 | 仅适用于纵向模式 |
| 页面渲染 | ❌ 需重构 | 目前直接调用 `pdf_*` 命令 |
| 缓存管理 | ⚠️ 需适配 | PDF 使用 Canvas，EPUB 使用 DOM |
...
| 组件 | 可复用性 | 说明 |
|------|----------|------|
| `CacheManager` | ⚠️ 需泛化 | 可抽象为通用缓存 |
| `PreloadPredictor` | ✅ 可复用 | 预加载预测算法 |
| `PerformanceMonitor` | ✅ 可复用 | 性能监控 |
| `PdfEngine` | ❌ PDF 专用 | 但模式可借鉴 |

### 4.3 现有问题

1. **前端**: `Reader.tsx` 直接调用 `invoke('pdf_render_page_to_file', ...)` 等命令，没有抽象层
2. **后端**: PDF 代码位于 `src-tauri/src/pdf/`，命令位于 `pdf_commands.rs`，需要重组目录结构
3. **文件类型**: `fileTypes.ts` 硬编码 `['.pdf']`

---

## 5. 重构计划

### 5.1 Phase 0: 准备工作（不影响现有功能）
...
mkdir -p src/services/formats/txt
...
mkdir -p src-tauri/src/formats/txt
mkdir -p src-tauri/src/formats/common
```

#### Step 0.2: 定义接口文件

- 创建 `src/services/formats/types.ts`
- 创建 `src-tauri/src/formats/mod.rs`

### 5.2 Phase 1: PDF 代码迁移（保持功能不变）

#### Step 1.1: 后端 PDF 代码迁移

1. 将 `src-tauri/src/pdf/*` 移动到 `src-tauri/src/formats/pdf/`
2. 更新 `mod.rs` 中的导出路径
3. 保持 `pdf_commands.rs` 暂时不变（保证兼容）

#### Step 1.2: 前端 PDF 渲染器封装

1. 创建 `src/services/formats/pdf/PdfRenderer.ts`，实现 `IBookRenderer` 接口
2. 将 `Reader.tsx` 中的 PDF 相关调用封装到 `PdfRenderer` 中
3. `Reader.tsx` 改为调用渲染器接口

### 5.3 Phase 2: 实现 Markdown 支持
...
> **注意**: `md-editor-rt` 是 `md-editor-v3` 的 React 版本，提供相同的功能但适配 React 生态。

#### Step 2.2: 后端 Markdown 引擎
...
### 5.5 Phase 4: 实现 MOBI/AZW3/FB2 支持

#### Step 4.1: 集成 foliate-js

1. 下载 `foliate-js` 源码到 `public/lib/foliate-js/`
2. 创建适配器 `FoliateRenderer.ts`
...
| 阶段 | 内容 | 预估工时 | 优先级 |
|------|------|----------|--------|
| Phase 0 | 接口定义 & 目录结构 | 2h | P0 |
| Phase 1 | PDF 重构（封装接口） | 4h | P0 |
| Phase 2 | Markdown 支持 | 3h | P0 |
| Phase 3 | EPUB 支持 | 6h | P0 |
| Phase 4 | MOBI/AZW3/FB2 支持 | 8h | P1 |
| Phase 5 | TXT 支持 | 3h | P2 |
| Phase 6 | 测试 & 优化 | 4h | P2 |
...
- [x] 创建 `src/services/formats/types.ts` 接口定义
- [x] 创建 `src-tauri/src/formats/mod.rs` trait 定义
- [x] 更新 `src/constants/fileTypes.ts` 支持新格式
- [x] 创建目录结构

#### Phase 1: PDF 重构 (4h) ✅ 已完成

- [x] 创建 `src/services/formats/pdf/PdfRenderer.ts`
- [x] 将 `Reader.tsx` 中 PDF 渲染逻辑迁移到 `PdfRenderer`
- [x] 创建渲染器工厂 `src/services/formats/index.ts` 和 `registry.ts`
- [x] 重构 `Reader.tsx` 使用 `IBookRenderer` 接口
- [x] 创建 `src/hooks/useBookRenderer.ts` 渲染器管理 Hook
- [x] 测试 PDF 阅读功能正常（需手动测试）

#### Phase 2: Markdown 支持 (3h) ✅ 已完成

- [x] 安装 `md-editor-rt` npm 包
- [x] 实现 `src-tauri/src/formats/markdown/mod.rs`（文件读取 + 编码检测）
- [x] 添加 `markdown_load_document` 命令
- [x] 实现 `src/services/formats/markdown/MarkdownRenderer.ts`
- [x] 配置主题支持（light/dark/sepia）
- [ ] 测试 Markdown 阅读（包含代码高亮、表格、图表等）- 需手动测试

#### Phase 3: EPUB 支持 (6h)

- [ ] 安装 `epub.js` npm 包
- [ ] 添加 Rust 依赖: `epub` (可选，用于元数据)
- [ ] 实现 `src/services/formats/epub/EpubRenderer.ts`
- [ ] 适配目录结构（EPUB 使用 NCX/Nav）
- [ ] 处理 EPUB 内置资源（CSS/图片）
- [ ] 测试 EPUB 阅读

#### Phase 4: MOBI/AZW3/FB2 支持 (8h)

- [ ] 下载并集成 `foliate-js`
- [ ] 实现 `src/services/formats/foliate/FoliateRenderer.ts`
- [ ] 处理不同格式的差异
- [ ] 测试各格式阅读

#### Phase 5: TXT 支持 (3h)

- [ ] 添加 Rust 依赖: `chardetng`, `encoding_rs`
- [ ] 实现 `src-tauri/src/formats/txt/mod.rs`
- [ ] 添加 `txt_load_document` 命令
- [ ] 实现 `src/services/formats/txt/TxtRenderer.ts`
- [ ] 测试 TXT 阅读（包含中文 GBK 编码测试）
...
重构 PDF 确保接口设计合理
  ↓
Markdown 实现简单，用于验证架构（纯前端渲染）
  ↓
EPUB 是最常用的电子书格式
  ↓
MOBI/AZW3/FB2 使用统一方案
  ↓
TXT 作为基础格式最后实现
  ↓
整体测试与优化
...
import { TxtRenderer } from './txt/TxtRenderer';
...
### 7.4 后端 Cargo.toml 依赖

```toml
# 新增依赖
chardetng = "0.1"          # TXT 编码检测
encoding_rs = "0.8"        # 编码转换
epub = "2.1"               # EPUB 解析（可选）
quick-xml = "0.36"         # FB2 XML 解析（可选）
mobi = "0.4"               # MOBI 解析（可选）
```
...
- EPUB 3.x 某些特性（如 Media Overlays）可能不被 `epub.js` 完全支持
- MOBI/AZW3 的 DRM 保护文件无法打开
- 某些 TXT 文件可能使用非常规编码

### 8.2 性能

- EPUB/MOBI 等格式在首次打开时需要解压和解析，可能较慢
- 建议显示加载进度

### 8.3 移动端适配

- `foliate-js` 在移动端 WebView 中的兼容性需要测试
- EPUB 渲染可能消耗较多内存
...
本方案通过定义统一的 `IBookRenderer` 接口，使 `Reader.tsx` 与具体格式解耦。核心思路：

1. **前端接口抽象**: 所有格式实现统一的 `IBookRenderer` 接口
2. **后端职责明确**: 负责文件读取、元数据提取、编码处理
3. **渐进式重构**: 先迁移 PDF，验证架构后再添加新格式
4. **复用现有逻辑**: UI 状态、翻页、书签等逻辑完全复用

执行顺序：**接口定义 → PDF 重构 → Markdown → EPUB → MOBI/AZW3/FB2 → TXT**
