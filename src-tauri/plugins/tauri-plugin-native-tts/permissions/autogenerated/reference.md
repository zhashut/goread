...
`native-tts:allow-install-tts-data`

</td>
<td>

Enables the install_tts_data command without any pre-configured scope.
...
`native-tts:deny-install-tts-data`

</td>
<td>

Denies the install_tts_data command without any pre-configured scope.
...
`native-tts:allow-open-tts-settings`
...
`native-tts:deny-open-tts-settings`
...
`native-tts:allow-all`

</td>
<td>

允许前端调用 native-tts 插件命令，并订阅插件事件。
