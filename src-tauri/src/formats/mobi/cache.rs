...
/// 缓存根目录
fn mobi_cache_root() -> PathBuf {
    if let Ok(dir) = std::env::var("GOREAD_MOBI_CACHE_ROOT") {
...
fn mobi_cache_root_legacy() -> PathBuf {
    let mut dir = std::env::temp_dir();
    dir.push("goread_cache");
    dir.push("mobi");
    dir
...
/// 章节缓存目录
fn mobi_section_cache_dir(book_hash: &str) -> PathBuf {
...
/// 资源缓存目录
fn mobi_resource_cache_dir(book_hash: &str) -> PathBuf {
    let mut dir = mobi_cache_root();
    dir.push("resources");
    dir.push(book_hash);
    dir
...
/// 元数据缓存目录
fn mobi_metadata_cache_dir() -> PathBuf {
    let mut dir = mobi_cache_root();
    dir.push("metadata");
    dir
}
...
impl MobiCacheManager {
    ...
    pub async fn save_section(
        ...
        let cache_dir = mobi_section_cache_dir(&book_hash);
    ...
    pub async fn clear_book_cache(&self, book_id: &str) -> Result<(), String> {
        ...
        // 清理章节缓存
        let section_dir = mobi_section_cache_dir(&book_hash);
        if section_dir.exists() {
            let _ = fs::remove_dir_all(&section_dir).await;
        }

        // 清理资源缓存
        let resource_dir = mobi_resource_cache_dir(&book_hash);
    ...
    pub async fn save_metadata(
        ...
        let book_hash = compute_book_hash(book_id);
        let cache_dir = mobi_metadata_cache_dir();
...
impl Clone for MobiCacheManager {
    fn clone(&self) -> Self {
        Self {
...
impl Default for MobiCacheManager {
    fn default() -> Self {
        Self::new()
    }
}
