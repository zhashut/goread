...
pub async fn delete_group(group_id: i64, delete_local: bool, db: DbState<'_>) -> Result<(), Error> {
    ...
    if delete_local {
        let paths: Vec<(String,)> = sqlx::query_as(
            "SELECT file_path FROM books WHERE group_id = ?"
        )
        .bind(group_id)
        .fetch_all(&*pool)
        .await?;
    ...
    while let Some(current_dir) = dirs_to_scan.pop_front() {
        ...
        while let Some(entry) = entries.next_entry().await? {
            ...
            *scanned_count += 1;
            ...
            // 每100个文件或每500ms发送一次进度更新
            if let Some(app) = app_handle {
                let should_emit = *scanned_count % 100 == 0 || last_emit_time.elapsed().as_millis() > 500;
                if should_emit {
                    let pdf_count = results.len() as u32;
                    let _ = app.emit("goread:scan:progress", serde_json::json!({
...
pub async fn scan_pdf_files(
    ...
    scan_pdf_files_recursive(&root, &mut results, &mut scanned_count, Some(&app_handle))
        .await
        .map_err(|e| format!("扫描失败: {}", e))?;

    // 发送最终结果
    let _ = app_handle.emit("goread:scan:progress", serde_json::json!({
        "scanned": scanned_count as u32,
...
pub async fn get_root_directories() -> Result<Vec<FileEntry>, String> {
    ...
    #[cfg(target_os = "ios")]
    let roots = vec![
        PathBuf::from("/private/var/mobile/Documents"),
    ];
