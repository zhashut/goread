...
use crate::formats::epub::EpubCacheManager;
...
/// 单批次最多跨越的章节数，避免一次性加载过多章节
const MAX_SECTIONS_PER_BATCH: i32 = 12;

/// EPUB 取片入口：从章节缓存按 cursor 续传地拉取 segments
pub async fn get_segments<R: Runtime>(
    ...
    let manager: Arc<Mutex<EpubCacheManager>> = app
        .state::<EpubCacheState>()
        .inner()
        .clone();
    ...
    while section_index < total_sections {
        ...
        let id_prefix = format!("epub:{}", section_index);
...
/// 从 cursor / startPosition / fallbackSection 解析起始章节与 chunk
...
/// 仅当 cursor 为空且首章节命中起始位置时，按 anchor 裁掉用户已读前缀
